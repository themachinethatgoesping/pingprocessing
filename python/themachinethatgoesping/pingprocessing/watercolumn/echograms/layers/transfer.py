...
from .layer import Boundary, Layer


def transfer_layer(src, dst, name: str, *, reference: str = "Depth (m)",
                   new_name: Optional[str] = None, mask: Optional[str] = None,
                   combine: bool = False) -> Layer:
    ...
    """
    lower, upper = src.get_layer_bounds(name, reference)
    times = src.ping_times
    target = new_name or name
    layer = Layer(
        reference,
        Boundary.time_series(times, lower),
        Boundary.time_series(times, upper),
        name=target,
    )
    dst.add_layer(target, layer, combine=combine)
    if mask is not None:
        for spec in dst.layers.specs(mask):
            dst.add_layer(target, spec, combine=True)
    return layer


def transfer_layers(src, dst, names: Optional[Iterable[str]] = None, *,
                    reference: str = "Depth (m)", mask: Optional[str] = None,
                    combine: bool = False) -> List[Layer]:
    ...
    """
    if names is None:
        names = src.layer_names()
    return [
        transfer_layer(src, dst, name, reference=reference, mask=mask, combine=combine)
        for name in names
    ]
